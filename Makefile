...
test:
	...
	$(ARTISAN) cache:clear
	$(PHP_SHELL) 'cd /var/www/html/app && export APP_ENV=testing DB_DATABASE=bird_test DB_HOST=mysql DB_PORT=3306 && if [ -f ./vendor/bin/pest ]; then ./vendor/bin/pest $(ARGS); elif [ -f ./vendor/bin/phpunit ]; then ./vendor/bin/phpunit $(ARGS); elif php artisan list --raw | grep -q "^test"; then php artisan test $(ARGS); else echo "No test runner available. Install dev dependencies." && exit 1; fi'
