...
- **Date and time display formatting** is governed by `app/docs/BLADE_GUIDELINES.md` (pre-format in controllers, never in Blade; use `config('display.time')` / `config('display.datetime')` for all user-visible times). Those rules apply to DataTable transformers, API Resources, and mail templates too — not just Blade.
