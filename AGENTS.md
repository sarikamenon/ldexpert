...
> **See `app/docs/TIMEZONE_GUIDE.md`** for the full reference — conversion-service API, worked examples (the NYC day-boundary case), the two date-column flavors, and edge cases. The imperatives below are enforced in addition to that document.
...
- **Display formatting** is governed by `app/docs/BLADE_GUIDELINES.md` (pre-format in controllers, `config('display.time')` / `config('display.datetime')`) — applies to transformers, Resources, and mail too.
...
- **Blade view rules** (including the ban on inline `<script>` blocks, `@php` data-shaping, JSON data islands, and the form-control color tokens) live in `app/docs/BLADE_GUIDELINES.md`. Read it before writing or modifying any `.blade.php` file.
...
UI rules — typography, spacing, interactive states, accessibility, responsiveness, empty states, destructive-action confirmation, color tokens, form structure, Blade components — all live in `app/docs/BLADE_GUIDELINES.md`. The full design-system reference is `app/docs/DESIGN_SYSTEM.md`; known gaps are tracked in `app/docs/DESIGN_PRINCIPLES_GAP_ANALYSIS.md`.
...
**Frontend / Blade**
- [ ] Ran through the Blade checklist in `app/docs/BLADE_GUIDELINES.md` (no `@php` data-shaping, no `<script>` blocks, design tokens only, form structure correct, etc.)
