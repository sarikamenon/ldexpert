...
*   **Source:** `qa/LD-Expert-QA.xlsx` (contains sheets for Admin, Therapist, Student, Finance, E2E).
...
*   **Result Reporting:** Run `/qa-admin`, `/qa-therapist`, `/qa-student`, `/qa-finance`, or `/qa-e2e` commands to execute subsets of tests. Each run publishes a timestamped `.md` + `.html` report pair into the unified `qa/reports/` folder (named `<suite>-YYYY-MM-DD-HHMM.{md,html}`), auto-generated from the run's JUnit output.
...
4.  **Full Report Generation**: Each suite run already emits a `.md` + `.html` report pair into `qa/reports/`. To produce an overall view, consolidate the latest per-suite reports in `qa/reports/` into a single summary.
