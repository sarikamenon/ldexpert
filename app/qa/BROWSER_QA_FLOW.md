...
│ └─ Output: app/qa/admin/test-plan.md + Excel rows              │
└────────────────┬────────────────────────────────────────────┘
                 │
┌─────────────────────────────────────────────────────────────┐
...
│ └─ Output: app/qa/reports/role-YYYY-MM-DD-HHMM.html            │
└────────────────┬────────────────────────────────────────────┘
                 │
┌─────────────────────────────────────────────────────────────┐
...
│ ├─ Fix tests if needed                                      │
│ └─ Commit: app/qa/LD-Expert-QA.xlsx + app/qa/CHANGELOG.md           │
└─────────────────────────────────────────────────────────────┘
```
...
# Writes: app/qa/admin/test-plan.md
# Appends: Rows to app/qa/LD-Expert-QA.xlsx
```
...
### CI/CD (Automatic)
```yaml
# Runs nightly at 08:00 UTC via GitHub Actions, on the staging server
.github/workflows/browser-qa-staging.yml
├─ Triggers: schedule (nightly) + manual dispatch
├─ Runs on: staging server via SSH (no Docker stack in CI)
├─ Runner script: scripts/ci/run-browser-qa-staging.sh
├─ Database: Fresh migrate on bird_test (hard-aborts otherwise)
├─ Tests: Runs all BrowserQA tests
└─ Reports: GitHub artifacts (HTML report + JUnit + screenshots)

# Docker-based fallback — MANUAL trigger only
.github/workflows/browser-qa.yml → scripts/ci/run-browser-qa.sh
```
...
6. Integrated with CI/CD (nightly runs on the staging server + on-demand)
