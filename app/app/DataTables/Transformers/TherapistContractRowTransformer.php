...
final class TherapistContractRowTransformer
{
    ...
    public static function transform(TherapistContract $contract): array
    {
        ...
        $actions = '<div class="flex space-x-1">'
            ...
            .'<a href="'.e(route('admin.contracts.therapists.edit', $contract)).'" class="inline-flex items-center justify-center w-8 h-8 bg-primary text-primary-foreground rounded hover:bg-primary/90 transition-colors" title="Edit Contract">'.$iconEdit.'</a>'
            .'<button type="button" class="therapist-contract-status-toggle inline-flex items-center justify-center w-8 h-8 rounded transition-colors bg-warning text-warning-foreground hover:bg-warning/90" data-endpoint="'.e($statusToggleUrl).'" data-next-status="'.e($nextStatus).'" title="'.e($toggleTitle).'">'
