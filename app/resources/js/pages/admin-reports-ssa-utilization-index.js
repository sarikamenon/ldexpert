...
async function initUtilizationTable() {
    ...
    if (dt && dt.on) {
        dt.on('xhr.dt', function (e, settings, json) {
            ...
            if (el('metricTotalTho')) el('metricTotalTho').textContent = Number(s.total_tho_hours || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            if (el('metricTotalServed')) el('metricTotalServed').textContent = Number(s.total_served_hours || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
