...
async function initTable() {
    const table = document.getElementById('adminQglobRequestsTable');
    if (!table) return;
    ...
    const form = document.getElementById('adminQglobRequestsFiltersForm');

    await initServerSideDataTable('#adminQglobRequestsTable', dataUrl, {
        order: [[0, 'desc']],
        pageLength: 25,
    ...
    if (form) {
        const reload = () => {
            if (typeof window.jQuery !== 'undefined') {
                const dt = window.jQuery('#adminQglobRequestsTable').DataTable();
                if (dt?.ajax?.reload) {
                    dt.ajax.reload();
                }
            }
