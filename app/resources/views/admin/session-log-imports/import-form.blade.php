<x-admin.layouts.app>
    <x-page-title title="Import Session Logs" />
    ...
    <x-ui::card class="p-6 space-y-6">
        <div>
            <h2 class="text-xl font-semibold mb-2">Import Session Logs from CSV</h2>
            <p class="text-sm text-foreground/60">
                Upload a CSV file containing session log information. The file must include all required columns.
                Duplicate entries (matching reference IDs) will be skipped automatically.
                Rows flagged as errors by the source system will also be skipped.
        ...
        <div>
            ...
            <select name="import_type" id="import_type"
                ...
                @foreach ($importTypes as $type)
                    <option value="{{ $type->value }}" @selected(old('import_type', 'RSM') === $type->value)>
                        {{ $type->value }}
                    </option>
                @endforeach
            </select>
        ...
        <div id="templateInfo" class="bg-foreground/5 rounded-lg p-4 space-y-3">
            ...
            <div class="mt-4">
                <a href="{{ route('admin.session-logs.import.template', ['type' => 'RSM']) }}"
        ...
        {{-- Import Form --}}
        <form id="importForm" method="POST" action="{{ route('admin.session-logs.import.store') }}"
            enctype="multipart/form-data" class="space-y-4">
            @csrf

            <input type="hidden" name="type" id="type" value="RSM" />

            <div>
                <x-input-label for="file" value="CSV File *" />
                <p class="mt-1 text-xs text-foreground/60" id="file_help">
                    Select a CSV file containing session log data. Maximum file size: 10MB.
        ...
        <div class="mt-4 pt-4 border-t border-border">
            <a href="{{ route('admin.session-logs.imports.index') }}"
    ...
    <x-slot name="scripts">
        @vite(['resources/js/pages/admin-session-log-import-form.js'])
    </x-slot>
</x-admin.layouts.app>
