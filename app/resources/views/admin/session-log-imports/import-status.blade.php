    ...
    <x-ui::card class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div class="bg-orange-50 rounded-lg p-4">
                <div class="text-sm text-orange-700">Skipped</div>
                <div class="text-2xl font-semibold text-orange-700">{{ $stats['skipped'] }}</div>
            </div>
        ...
        <div class="border-t border-border pt-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex gap-2">
                    ...
                    <button id="filterSkipped"
                        class="px-3 py-1 text-xs border rounded-md hover:bg-foreground/5">Skipped</button>
                    ...
                    <tbody id="rowsTableBody" class="bg-white divide-y divide-border">
                        @foreach ($import->rows as $row)
                            <tr class="row-item" data-status="{{ $row->status->value }}">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-foreground/70">
                                    {{ $row->reference_id ?? '—' }}
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span
                                        ...
                                        @elseif($row->status->value === 'skipped') bg-orange-100 text-orange-800
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm">
                                    @if ($row->session_log_id && $row->sessionLog)
                                        <a href="{{ route('admin.session-logs.show', $row->sessionLog) }}"
                                            class="text-primary hover:text-primary/80">
                                            Log #{{ $row->sessionLog->id }}
                                        </a>
                                    @else
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-border">
            ...
            <a href="{{ route('admin.session-logs.import') }}"
