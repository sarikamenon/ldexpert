<x-admin.layouts.app>
    <x-ui::show-header title="Edit Expense" :subtitle="'Expense #' . $expense->id" :back-url="route('admin.expenses.show', $expense)"
        back-label="Back to Expense" />
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-foreground/70">$</span>
                            <input type="number" name="amount" step="0.01" min="0.01"
                                value="{{ old('amount', $expense->amount) }}" required
                                class="w-full pl-8 pr-3 py-2 border border-border rounded-md focus:ring-2 focus:ring-primary">
                        </div>
                        @error('amount')
                            <p class="text-sm text-danger mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="vendor_payee" value="{{ old('vendor_payee', $expense->vendor_payee) }}"
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" placeholder="What was this expense for?"
                        class="w-full px-3 py-2 border border-border rounded-md focus:ring-2 focus:ring-primary">{{ old('description', $expense->description) }}</textarea>
                    @error('description')
                        <p class="text-sm text-danger mt-1">{{ $message }}</p>
                    @enderror
                </div>
