<x-admin.layouts.app>
    <x-slot name="scripts">
        @vite(['resources/js/pages/admin-therapist-bills-attach-sessions.js'])
    </x-slot>
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-foreground/60 mt-1">
                    Bill {{ $bill->bill_number }} for {{ $bill->therapist->name }}
                </p>
            </div>
    ...
    <x-ui::card class="p-6 mb-6">
        <form method="GET" action="{{ route('admin.billing.therapist-bills.attach-sessions', $bill) }}" id="filterForm"
            class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <div class="space-y-1">
                    ...
                    <p class="mt-1 text-xs text-foreground/60" id="filter_student_id_help">
                        Filter by student.
                    </p>
                    <x-ui::select id="filter_student_id" name="student_id" class="mt-1 block w-full" searchable placeholder="All Students"
                        aria-describedby="filter_student_id_help">
                        <option value="">All Students</option>
                        @foreach ($students ?? [] as $s)
                            <option value="{{ $s->id }}" @selected(($filters['student_id'] ?? null) == $s->id)>
                                {{ $s->name }}
                            </option>
                        @endforeach
                    </x-ui::select>
                ...
                <div class="space-y-1">
                    ...
                    <p class="mt-1 text-xs text-foreground/60" id="filter_service_id_help">
                        Filter by service type.
                    </p>
                    <x-ui::select id="filter_service_id" name="service_id" class="mt-1 block w-full" searchable placeholder="All Services"
                        aria-describedby="filter_service_id_help">
                        <option value="">All Services</option>
                        @foreach ($services ?? [] as $svc)
                            <option value="{{ $svc->id }}" @selected(($filters['service_id'] ?? null) == $svc->id)>
                                {{ $svc->name }}
                            </option>
                        @endforeach
                    </x-ui::select>
                ...
                <div class="space-y-1">
                    ...
                    <p class="mt-1 text-xs text-foreground/60" id="filter_date_from_help">
                        Start of session date range.
                    </p>
                    <x-ui::input type="date" id="filter_date_from" name="date_from" class="mt-1 block w-full"
                        value="{{ $filters['date_from'] ?? $bill->billing_period_start?->format('Y-m-d') }}" aria-describedby="filter_date_from_help" />
                </div>

                <div class="space-y-1">
                    <x-input-label for="filter_date_to" value="Date to" />
                    <p class="mt-1 text-xs text-foreground/60" id="filter_date_to_help">
                        End of session date range.
                    </p>
                    <x-ui::input type="date" id="filter_date_to" name="date_to" class="mt-1 block w-full"
                        value="{{ $filters['date_to'] ?? $bill->billing_period_end?->format('Y-m-d') }}" aria-describedby="filter_date_to_help" />
                </div>
            </div>
    ...
    <form method="POST" action="{{ route('admin.billing.therapist-bills.attach-sessions.store', $bill) }}" id="attachSessionsForm">
        @csrf
        ...
        <x-ui::card class="p-6">
            ...
            <div id="sessionLogsSummary" class="p-4 bg-background/subtle rounded-lg hidden mb-4">
                        ...
                        <tbody>
                            @foreach ($sessionLogs as $log)
                                ...
                                <tr class="border-b border-border hover:bg-background/subtle session-log-row">
                                    ...
                                    <td class="py-3 px-4 text-sm font-medium">${{ number_format($log->therapist_billable_amount ?? 0, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
                <x-ui::empty-state
                    ...
                    description="No session logs match the current filters, or there are no available approved sessions to add."
                />
            @endif
            ...
            <div class="mt-6 flex flex-col items-end gap-4">
                <div class="flex items-center gap-4">
                    <a href="{{ route('admin.billing.therapist-bills.show', $bill) }}">
                        <x-ui::button variant="secondary">Back to bill</x-ui::button>
                    </a>
