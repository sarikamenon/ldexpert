<x-admin.layouts.app>
    <x-page-title title="Therapist Contract #{{ $contract->id }}" />

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <x-ui::card class="p-6 space-y-2">
            <p class="text-sm text-foreground/70">Therapist</p>
                ...
                <tbody>
                    @foreach ($contract->services as $service)
                        <tr class="border-b border-border/60 last:border-0">
                            ...
                            <td class="py-2 px-3">{{ $service->no_show_rate !== null ? '$' . number_format((float) $service->no_show_rate, 2) : '—' }}</td>
