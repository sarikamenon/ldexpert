...
<form method="POST" action="{{ $action }}" class="space-y-6">
    ...
    <div>
        ...
        <textarea id="objective" name="objective" rows="4" maxlength="5000" required
            class="mt-1 block w-full rounded-base border border-input bg-white px-3 py-2 text-foreground placeholder:text-foreground/50 focus:ring-2 focus:ring-ring focus:border-ring"
    ...
    <div class="flex items-center justify-end gap-3">
        @if ($cancelUrl)
            <a href="{{ $cancelUrl }}"
                class="inline-flex items-center rounded-md border border-border bg-white px-4 py-2 text-sm font-medium text-foreground hover:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                Cancel
            </a>
        @endif
