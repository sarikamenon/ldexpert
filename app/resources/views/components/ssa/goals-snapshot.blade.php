...
<x-ui::card {{ $attributes->merge(['class' => 'p-6 h-full']) }}>
    <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4 mb-4">
        <div>
            <h3 class="text-lg font-semibold text-foreground">Goals</h3>
            <p class="mt-1 text-xs text-foreground/60" id="ssa-goals-snapshot-help">
                Goal progress for this service agreement.
            </p>
        </div>
        @if ($goalsTabUrl)
            <a href="{{ $goalsTabUrl }}"
                class="inline-flex shrink-0 items-center justify-center rounded-md border border-border bg-background px-3 py-1.5 text-sm font-medium text-primary shadow-sm transition-colors hover:bg-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background active:bg-muted/80"
                aria-describedby="ssa-goals-snapshot-help">
                View all goals
    ...
    @if ($hasNoGoals)
        <div class="rounded-xl border border-border bg-muted/25 p-4">
            <div class="flex flex-col items-center justify-center py-6 text-center">
                <div class="mb-3 inline-flex h-12 w-12 items-center justify-center rounded-full bg-muted">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-foreground/50" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" />
                    </svg>
                </div>
                <p class="text-sm font-medium text-foreground/70">No goals yet</p>
                <p class="mt-1 text-xs text-foreground/50">Add goals to track student progress.</p>
            </div>
        </div>
    @else
        <div class="rounded-xl border border-border bg-muted/25 p-4">
            <div class="grid grid-cols-2 gap-3">
                <div class="group flex min-h-[5rem] flex-col justify-between rounded-lg border border-border bg-card p-3 shadow-sm transition-colors hover:border-primary/25 hover:bg-muted/30 focus-within:border-primary/25">
                ...
                <div class="group flex min-h-[5rem] flex-col justify-between rounded-lg border border-border bg-card p-3 shadow-sm transition-colors hover:border-success/30 hover:bg-success/5 focus-within:border-success/30">
                    <div class="flex items-center justify-between gap-2">
                ...
                <div class="group flex min-h-[5rem] flex-col justify-between rounded-lg border border-border bg-card p-3 shadow-sm transition-colors hover:border-primary/30 hover:bg-primary/5 focus-within:border-primary/30">
                    <div class="flex items-center justify-between gap-2">
                ...
                <div class="group flex min-h-[5rem] flex-col justify-between rounded-lg border border-border bg-card p-3 shadow-sm transition-colors hover:border-warning/30 hover:bg-warning/5 focus-within:border-warning/30">
                    <div class="flex items-center justify-between gap-2">
            ...
            <div class="mt-3 flex flex-col justify-between rounded-lg border border-border bg-card p-4 shadow-sm">
