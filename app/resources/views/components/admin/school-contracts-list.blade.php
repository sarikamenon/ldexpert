...
<x-ui::card class="p-6 space-y-4">
    <div class="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <form method="GET" class="flex flex-wrap gap-2">
            @if($context !== 'detail')
            <x-text-input type="text" name="search" class="w-64" placeholder="Search by ID or School"
                value="{{ $filters['search'] ?? '' }}" />
            @else
             {{-- In detail view, we might not need to search by school name, but ID search is still useful --}}
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex gap-2">
                                ...
                                <button type="button"
                                    class="contract-status-toggle inline-flex items-center px-3 py-1.5 rounded-md text-sm {{ $contract->status === \App\Enums\ContractStatus::ACTIVE ? 'bg-danger text-danger-foreground hover:bg-danger/90' : 'bg-success text-success-foreground hover:bg-success/90' }}"
